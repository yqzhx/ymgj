...
$(document).ready(function () {
    ...
    jQuery.validator.addMethod("isTelphone",function (value, element) {
        var length = value.length;
        var Telphone=/^((1[3|4|5|6|7|8][0-9]\d{8})|((0\d{2,3}-)?(\d{7,8}-)?(\d{1,4})))|((0\d{2,3}-)?(\d{7,8}))$/;
        return this.optional(element)||(6<length && length<18 &&Telphone.test(value));
    },"请填写正确的手机号码或座机号码");
    //姓名校验
    ...
    jQuery.validator.addMethod("isLoginName", function (value, element) {
        var name = /^[\u0391-\uFFE5A-Za-z]{4,20}$/;
        return this.optional(element) || (name.test(value));
    }, "账号名只能由字母和汉字组成，且长度为4-20位");
    //校验账号名
    ...
    //校验是否为价格(两位小数点以内正整数，包括0)
    jQuery.validator.addMethod("isMoney", function (value, element, param) {
        var exg = /^([1-9]{1}[0-9]{0,3}(\,[0-9]{3,4})*(\.[0-9]{0,2})?|[1-9]{1}\d*(\.[0-9]{0,2})?|0(\.[0-9]{0,2})?|(\.[0-9]{1,2})?)$/
        if (exg.test(value)) {
            return true;
        } else {
            return false;
