    ...
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                ...
                _fullPage = options.fullPage || true;


                var fixedHeader = $('#bootstrap-table').data('fixedheader');
                console.log(fixedHeader)
                var bt = $('#bootstrap-table').bootstrapTable({
                    height:fixedHeader?'800':'auto',
                    ...
                    toolbar: options.toolbar == 0?undefined:'#toolbar',                                // 指定工作栏
                    ...
                    onLoadSuccess:function(data){
                        setTimeout(function(){
                            fixedHeader&&$.table.tableHeight();
                        },0)

                        //ajax加载完成回调
                        if($.table._option.cb){
                            $.table._option.cb(data);
                        }
                    },
                    onEditableSave:function(field,row,oldValue,$el){
                        options.onEditableSave&&options.onEditableSave(field,row,oldValue,$el)
                    }
                });
            },
            tableHeight:function(){
                var contentBox = $('.bootstrap-table').closest('.content-box');
                var searchCollapse = $('.search-collapse');
                if(contentBox.length<=0) return;

                // 使table占满全屏
                var contentBoxHeight = contentBox.outerHeight(false);
                var contentBoxHeightPaddingTop = parseInt(contentBox.css('paddingTop'));
                var contentBoxHeightpaddingBottom = parseInt(contentBox.css('paddingBottom'));
                var contentBoxInnerHeight = contentBoxHeight - contentBoxHeightPaddingTop - contentBoxHeightpaddingBottom;
                var searchCollapseHeight = searchCollapse.outerHeight(true);
                var tableHeight = contentBoxInnerHeight-searchCollapseHeight;
                console.log('contentBoxInnerHeight')
                console.log(contentBoxInnerHeight)
                console.log('searchCollapseHeight')
                console.log(searchCollapseHeight)
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == value) {
                        ...
                        actions.push("<span class='text-"+dict.listClass+"'>"+dict.dictLabel+"</span>")
                        return false;
                    }
                });
                return actions.join('');
            ...
            selectUserName: function(datas, value) {
                var actions = [];
                $.each(datas, function(index, dict) {
                    if (dict.loginName == value) {
                        actions.push("<span>"+dict.userName+"</span>");
                        return false;
                    }
                });
                return actions.join('');
        ...
        form: {
            ...
            reset: function(formId) {
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                $("#" + currentId)[0].reset();
                $("#" + currentId).find('input[type=hidden]').each(function(index,item){
                    $(item).val('');
                })
                $("#" + currentId + " select").each(function(){
        ...
        modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height,btnList) {
                //如果是移动端，就使用自适应大小弹窗
                ...
                // btnList为null时， 显示‘确定’‘关闭’两个按钮，确定触发页面内submitHandler方法
                // 为1时，不显示按钮
                if($.common.isEmpty(btnList)) {
                    $.extend(options,{
                        btn:['确定', '关闭'],
                ...
                function getParentIframe(){
                    // console.log('当前iframe')
                    var myIframe = $(parent.document).find('iframe[name='+window.name+']');
                    // console.log(myIframe)
                    return myIframe;
                }
                function getParentIframeName(){
                    // console.log('当前iframen的name')
                    return window.name;
                }
            },
            // open: function (title, url, width, height,isTop) {
            // 	//如果是移动端，就使用自适应大小弹窗
            // 	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            // 	    width = 'auto';
            // 	    height = 'auto';
            ...
            // 	if(isTop){
            //         // 获取顶层iframe的window对象
            //         var topIframeWindow = $(top.document).find('iframe:visible')[0].contentWindow;
            //         topIframeWindow.layer.open({
            //             type: 2,
            //             area: [width+'px', height+'px'],
            //             fix: false,
            //             //不固定
            ...
            openOptions: function (options) {
                ...
                var closeBtn = $.common.isEmpty(options.closeBtn) ? 1 : options.closeBtn;
                var shadeClose = $.common.isEmpty(options.shadeClose) ? false : options.shadeClose;
                var btn = $.common.isEmpty(options.btn) ? ['<i class="fa fa-check"></i> 确认', '<i class="fa fa-close"></i> 关闭'] : options.btn
                layer.open({
                    type: 2,
                    maxmin: true,
                    shade: 0.3,
        ...
        operate: {
            ...
            add: function(id,options) {
                ...
                var width = options ? options.width : null;
            ...
            edit: function(id,options) {
                ...
                if(options){
                    $.modal.open("修改" + $.table._option.modalName, url,options.width,options.height);
                }else{
            ...
            // 成功回调执行事件（父窗体静默更新）
            successCallback: function(result,pName) {
                var currentIframe = $('#content-main',top.document).find('iframe:visible');
                var currentWindow = currentIframe.length>0?currentIframe[0].contentWindow:top;
                var slibingsIframes = currentIframe.length>0?currentIframe.siblings():'';
                var index = parent.layer.getFrameIndex(window.name);
                var otherLayerIframes = $('.layui-layer-iframe:not(#layui-layer'+ index +')',top.document).find('iframe');
                // 接口成功
                if (result.code == web_status.SUCCESS) {

                    // 刷新所有弹出层iframe的table和tree-table
                    if(otherLayerIframes.length>0){
                        otherLayerIframes.each(function(index,item){
                            var win = $(item)[0].contentWindow;
                            if (win.$("#bootstrap-table").length > 0) {
                                // win.$.modal.msgSuccess(result.msg);
                                win.$.table.refresh();
                            } else if (win.$("#bootstrap-tree-table").length > 0) {
                                // win.$.modal.msgSuccess(result.msg);
                    ...
                    // 刷新#content-main下当前iframe下所有table和tree-table
                    if(currentWindow.$("#bootstrap-table").length>0){
                        currentWindow.$.modal.msgSuccess(result.msg);
                        currentWindow.$.table.refresh();
                    }else if (currentWindow.$("#bootstrap-tree-table").length > 0) {
                        currentWindow.$.modal.msgSuccess(result.msg);
                        currentWindow.$.treeTable.refresh();
                    } else {
                        // $.modal.msgReload("保存成功,正在刷新数据请稍后……", modal_status.SUCCESS);
                    ...
                    // 刷新#content-main下当前iframe的所有同级iframe下所有table和tree-table
                    if(slibingsIframes.length>0) {
                        slibingsIframes.each(function (index, item) {
                            var win = $(item)[0].contentWindow;
                            if (win.$("#bootstrap-table").length > 0) {
                                // win.$.modal.msgSuccess(result.msg);
                                win.$.table.refresh();
            ...
            iframeReload:function(){
                var currentIframe = $('iframe:visible',top.document)[0].contentWindow;
